            ...
            <div class="w-full md:w-4/5 shadow-2xl p-4 my-8 bg-white rounded-md">
                <div class="relative pt-10 pb-5 px-2.5  border-t-2 border-blue-800">
                    <p class="text-3xl md:text-5xl text-center text-blue-900 uppercase">
                        TIÊU CHÍ CHẤM ĐIỂM THI ĐUA KHỐI {{ $unit->type == 'MN' ? "Mầm non" : ($unit->type == 'TH' ? "Tiểu học" : "Trung học cơ sở") }} TỪ 
                        <br>
                ...
                <div class="relative pt-10 pb-5 px-2.5 mt-6 border-t-2 border-blue-800">
                    ...
                    <div class="md:grid grid-rows-5 md:grid-rows-3 grid-flow-col gap-4">
                        <div class="md:row-span-3">
                            <img width="100px" src="{{ $unit->logo ? Voyager::image($unit->logo) : 'http://localhost/storage/users/default.png'}}" alt="">
                        </div>
                        <div class="md:col-span-2 ">
                            <div class="flex space-x-2 items-center">
                                <label for="name" class="font-bold text-black">Tên đơn vị:</label>
                                <p class="text-blue-600 font-medium">{{$unit->name}}</p>
                            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('evaluations', () => ({
                ...
                renderCategories(categories, isCriteria = false,parentIndex = '') {
                    let html = '';
                    let index = 1;
                    ...
                    categories.forEach(category => {
                        ...
                        html += `
                            <tr class="bg-white ">
                                <td class="px-6 py-4 font-bold  border dark:bg-gray-800 dark:border-gray-700">
                                    ${currentIndex == 1 ? this.convertToRoman(index) : currentIndex}
                                </td>
                                <th scope="row" class="px-6 py-4 font-${isCriteria ? 'normal' : 'medium text-gray-900 dark:text-white' }  border dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 border dark:bg-gray-800 dark:border-gray-700">
                                    <input type="number"  class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 " placeholder="Point" :value="${category.point ? category.point : category.total_point}" readonly>
                                </td>
                                ${isCriteria 
                                ? 
                                `
                                <td class="px-6 py-4 border dark:bg-gray-800 dark:border-gray-700">
                                    <input type="number" name="point[${category.evaluation_id}]" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Điểm" ></td>
                                <td class="px-6 py-4  border dark:bg-gray-800 dark:border-gray-700">
                                    <input type="file" name="file[${category.evaluation_id}]" class="w-full text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  p-2.5    dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                </td>    
                                `
                                :
                                `<td class="px-6 py-4 border dark:bg-gray-800 dark:border-gray-700"></td>
                                <td class="px-6 py-4 border dark:bg-gray-800 dark:border-gray-700"></td>`
                                }
                            </tr>
                        `;
                        ...
                        if (category.children) {
                            html += this.renderCategories(category.children, false,currentIndex);
                        }
                        if(category.criteria){
                            html += this.renderCategories(category.criteria , true,currentIndex);
